...
- **Split work with subagents.** PUI routes local repository evidence to *Explore*, external investigation to *Research*, and decided execution to *Worker*.
...
| `@gotgenes/pi-subagents` | Local evidence, external research, and execution need clear boundaries while the parent retains control. | Defines tool-restricted `Explore`, `Research`, and `Worker`; fails closed on unknown types; applies mapped models and inherited reasoning; permits 128 running and 512 queued background agents per Pi instance; and delivers completions at the next parent turn. |
