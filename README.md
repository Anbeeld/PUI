...
Each release pins its managed composition in [`stack.json`](stack.json). PUI structurally merges its owned values into most configuration files below. The reasoning-summary and session-title files are create-only: PUI writes defaults only when a file is missing and otherwise preserves a valid file byte-for-byte. The [patch reference](docs/patches.md) documents each lifecycle contract.
...
| `~/.config/pui/reasoning-summaries.json` | Create-only per-model `auto`, `concise`, `detailed`, or `none` modes; new files default all three GPT-5.6 variants to `detailed` |
| `~/.config/pui/session-titles.json` | Create-only ordered exact-or-fuzzy title-model selectors; the active session model is always the final fallback |
...
| `@gotgenes/pi-subagents` | Subagents should create substantial concurrency without outsourcing or duplicating the main critical path. | Defines background-default, tool-restricted `Explore`, `Research`, and `Worker`; delegates only distinct scoped deliverables when main has immediate non-overlapping work or at least two agents can run concurrently; names child-owned and excluded files, sources, and actions plus reserved main work; requires main to do that work before waiting and prevents duplicate active evidence or checks; keeps sequential work, choices, verdicts, and sole-critical-path follow-ups in main; treats missing handoff fields, stopped, limit-steered, missing-coverage, and residual-work results as incomplete; propagates provider errors with partial output instead of false completion and rejects blank resolved responses; preflights tools and readable inputs; preserves authority/project instructions and one skill catalog; fails closed on unknown types; applies mapped models and inherited reasoning with explicit invocation precedence; bounds background capacity; and delivers completions at the next parent turn. |
| PUI skill loader | Skill descriptions alone do not reliably cause every model to read applicable instructions before acting. | Installs a lightweight `load_skill` tool that loads an exact listed skill on demand, exposes only its direct Markdown reference filenames, and returns one or more selected references together with their parent skill when needed. Its prompt directs skill loading at task start and material phase changes, including matching parts of mixed tasks and required references. |
| `@99percentpeople/pi-background-tasks` | Overlapping guidance obscures tool selection, and module-global runtime state leaks across cached extension instances. | Replaces prompt/schema metadata with compact guidance and isolates mutable runtime state per extension instance. |
...
| Responses reasoning summaries | Safe summary text should be generated at a user-selected detail level and remain readable without exposing raw reasoning or encrypted replay data. | Installs a request hook that applies exact provider/model overrides before bare model IDs for the three Responses adapters, defaults the three GPT-5.6 variants to `detailed`, treats `none` as omission, and never creates reasoning when it is off. The display patch trusts the parser-owned transient marker only in explicitly live projections, requires validated `summary_text` signatures elsewhere, renders entirely bold summaries as italics, leaves stored messages unchanged, and transactionally rewinds failed cross-runtime snapshot restoration. |
| Automatic session titles | New sessions should receive useful compact names without delaying their main work or recreating a full agent. | Starts a detached title request from the first expanded user prompt, tries at most four configured exact-or-fuzzy model selectors in order with two provider retries, falls back to the active session model, requests no reasoning when supported, otherwise selects the model's lightest advertised reasoning level, and uses a 1,024-token ceiling plus a direct-prohibition prompt. The request remains one short JSON-framed user message with no system prompt or tools; input is capped at 600 Unicode code points and titles at 80, generation cancels with the run, and any explicit or concurrent manual name—including a Pi Web sidebar rename—wins. |
| Pi Web goal-command display | pi-goal's internal prompts should not replace or temporarily remove the concise command the user entered. | Projects only strictly recognized initial and explicit-resume prompts as canonical `/goal …` and `/goal resume` text in Pi Web; retains an optimistic start, edit, or resume command until its generated goal turn or idle recovery becomes authoritative regardless of event order; and leaves stored prompts and model context unchanged. |
...
| Pi Web presentation | The app needs PUI identity and icons, while upstream's footer leaves excess widget space and its tool-call headers hide reference arguments. | Applies branding and browser/PWA icons, versions affected caches, fixes the footer gap and duplicate border, and shows `skill: references` in tool-call headers for reference loads. |
...
| Pi Web updates | Pi Web's updater cannot safely update PUI's exact multi-package composition. | Adds PUI's update route, card, restart control, bridge, and inert identity extension; update-integration changes are transactional, installed updater files are hash-verified before code loads, Windows relaunches use the absolute hidden npm shim, completed restart status cannot suppress later discovery, and approved updates validate and roll back on failure. |

JSON changes use timestamped backups and preserve unrelated settings. The reasoning-summary and session-title files are never removed on uninstall and are never rewritten once valid. Version-anchored patches verify upstream shape after install and update; patch-specific backups and manifests expose drift and preserve user changes during rollback or uninstall. PUI-named autostart files are likewise removed only while their complete canonical content still matches.
