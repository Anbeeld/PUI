...
// pui-subagents-patch:main-session-model-v4
export class AgentTool {
	...
	toToolDefinition() {
		...
		return defineTool({
			...
			label: "Subagent",
			promptSnippet: "Launch a specialized agent using the user-configured fuzzy model mapping and the parent session's reasoning level unless the user explicitly overrides either.",
			description: `Launch a new agent to handle complex, multi-step tasks autonomously. By default, a subagent uses the user-configured fuzzy model mapping when one matches the parent, otherwise the parent model, and inherits the parent session's active reasoning level.
...
`,
			parameters: Type.Object({
				...
				thinking: Type.Optional(
					Type.String({
						description:
							"Reasoning-level override: off, minimal, low, medium, high, or xhigh. Set only when the user explicitly requests a different level; otherwise omit it to inherit the parent session's active reasoning level.",
					}),
				),
