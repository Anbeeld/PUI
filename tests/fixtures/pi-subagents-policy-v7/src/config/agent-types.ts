...
export class AgentTypeRegistry implements AgentConfigLookup {
  ...
  static readonly DEFAULT_AGENT_NAMES = ["Worker", "Explore", "Research"] as const;
  ...
  /** Resolve agent config with guaranteed non-null return. Falls back internally to Worker. */
  resolveAgentConfig(type: string): AgentConfig {
    const key = this.resolveKey(type);
    const config = key ? this.agents.get(key) : undefined;
    if (config) return config;

    const workerKey = this.resolveKey("Worker");
