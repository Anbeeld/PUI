...
- `lib/pui-background-tasks-patch.js` owns the PUI-managed, version-anchored replacement of `@99percentpeople/pi-background-tasks` tool descriptions, snippets, parameter descriptions, and prompt guidelines. Install/update apply and verify the exact transform after package reconciliation. A hashed ownership manifest permits migration from an older PUI-owned transform using the pristine `index.min.js.pui-original`; update transactions back up bundle, original, and manifest for rollback; staged target scripts retain the same snapshot under one transaction-level guard until outer validation finishes, including when the installed worker predates the patch; uninstall restores only an exact owned shape.
- `lib/pui-subagents-patch.js` owns the PUI-managed, version-anchored model and reasoning policy for `@gotgenes/pi-subagents`. It rewrites the `subagent` tool's model- and thinking-facing metadata, removes the built-in `Explore` haiku pin, ignores agent-profile model/thinking defaults during normal parented tool invocation, and reads fuzzy parent-to-child mappings from `~/.config/pui/subagents.json`. Both mapping keys and values use upstream's model resolver. An explicit invocation `model` or `thinking` wins; an unavailable mapped child falls back to the parent, and omitted thinking inherits the parent session's active level before Pi clamps it to child capabilities. Install/update reconcile new defaults from `stack.json` through `_pui.defaultMappings`: untouched defaults may advance, user overrides and deletions survive, and newly shipped pairs are appended. A hashed ownership manifest and per-file `.pui-original` backups support patch migration and rollback; update transactions back up source, original, manifest, and mapping-config files, staged target scripts retain the snapshot under a transaction-level guard, and uninstall restores only an exact owned source shape while preserving the user config.
- `lib/pui-pi-8782-backport.js` owns the temporary, exact-version Pi #8782 dist backport for the unbundled Pi runtime resolved by Pi Web. It patches only agent-loop scheduling and AgentSession between-turn compaction for Pi Web's pinned 0.84.3 runtime, reapplies after install/update, verifies through doctor, and restores owned files during uninstall; standalone `pi` remains the stock 0.84.3 bundle. The helper must be retired automatically when the Pi pin advances to a release containing #8782.
...
For lifecycle changes, also exercise the relevant repository script on the target platform when it is safe to do so. For the temporary Pi #8782 bridge, run `npm run release:verify-pi-8782` against an isolated exact npm install; it must also prove the patched runtime behavior before release. Never weaken or skip a failing assertion to obtain a pass; diagnose the mismatch or report the remaining gap.
