...
| `@99percentpeople/pi-background-tasks` retains the exact 2.1.1 metadata, import/export, and source-map anchors patched by PUI; its cached default factory isolates mutable state per Pi runtime; `node-pty` loads from bundled prebuilds and may be approved/rebuilt when unavailable | `piPackages`, `backgroundTasksPromptPatch`, `lib/pui-background-tasks-patch.js`, `lib/pui-native-check.js` | Run `npm run release:verify-prompt-patches` against the exact published artifact, including cached-factory snapshot-routing checks, then run owned-transform migration/rollback tests; run install/update on Windows, macOS, and Linux (x64/arm64), verify both helpers, and confirm a PTY spawns |
| `@gotgenes/pi-subagents` 19.3.5 retains the exact taxonomy/routing/capability, inherited-prompt filtering, unknown-type, result-integrity, incomplete-outcome classification, completion, mapped-model, reasoning, registry-overlay, and tool-schema patch anchors; child sessions bind `pi-web-access` extension tools before applying profile allowlists | `piPackages`, `subagents`, `subagentsPromptPatch`, `configPaths.puiSubagents`, `lib/pui-subagents-patch.js` | Run `npm run release:verify-prompt-patches` against the exact published artifacts. Its runtime harness verifies background-default profiles, explicit foreground override precedence, exact tool sets, case-insensitive custom/default overrides, added custom profiles, fail-closed unknown types, inherited-prefix prompt rendering, parallelism-gated route guidance, provider-error/partial/empty response rejection before lifecycle completion, limit-steered incomplete classification, mapping fallback/overrides, reasoning inheritance/schema, idempotency, and restoration. Run every known revision migration/removal, interruption recovery, restore-failure retention, unknown-manifest, and case-insensitive config reconciliation test; then exercise install/update on each platform |
| Pi and Pi Web both run `before_provider_request` with the active model identity in `ctx.model`, and the three managed Responses adapters accept `reasoning.summary` values `auto`, `concise`, and `detailed` while omission disables summaries | `reasoningSummaryExtension`, `reasoningSummaries`, `configPaths.puiReasoningSummaries`, `extensions/pui-reasoning-summary/` | Run the extension policy tests for exact qualified/bare precedence, Azure identity, every mode, unsupported APIs, invalid config, and reasoning-off requests; then exercise one terminal and one Pi Web request and inspect the provider payload |
| Pi runs awaited `before_agent_start` handlers after prompt expansion and exposes the active model, available model registry, session entries, session-name setters, and a run abort signal; a handler may detach a provider request without delaying the main loop, and Pi Web persists sidebar renames through a second `SessionManager` the running agent does not see | `sessionTitleExtension`, `sessionTitles`, `configPaths.puiSessionTitles`, `extensions/pui-session-title/` | Run title-policy tests for eligibility, concurrency, fuzzy ordered fallback with the candidate cap, request shape, retries, compact JSON-framed input, output cleanup including terminal-control stripping and post-truncation validation, race-safe persistence across both in-memory and persisted session names, deadline/shutdown cancellation, and failures; then start fresh terminal and Pi Web sessions with a deliberately slow title model and confirm main generation begins before each title completes and a rename during generation is not overwritten |
...
13. With the shipped exact mappings from `openai-codex/gpt-6-astra`, `openai-codex/gpt-5.6-sol`, and `openai-codex/gpt-5.6-terra` to `openai-codex/gpt-5.6-luna` in `~/.config/pui/subagents.json`, spawn from each parent without `model` or `thinking` and confirm resolution selects available Luna with the parent's active reasoning level. Make Luna unavailable and confirm fallback to each parent; then confirm explicit model and reasoning overrides. Change and delete mappings, run an update, and confirm those user choices survive while a newly shipped fixture mapping is appended. Repeat the invocation checks in both the terminal and Pi Web; from supported Claude and Gemini parents with no matching mapping, confirm parent-model and active-reasoning inheritance.
...
15. In fresh sessions with the exact parent/child model and reasoning settings recorded, run `tests/fixtures/pi-subagents-routing-eval.json` and inspect the complete trajectories. Trivial and substantial single-track work, dependent analysis followed by editing, sequential current-source research, decided sequential implementation, the overall plan, sole-critical-path audits, and delegated severity or verdicts must stay in main. One background agent is valid only while main immediately executes a named substantial non-overlapping track; its dispatch must name child-owned and excluded files, sources, or actions, and main must complete its reserved work before blocking. Two or more agents must receive distinct deliverables and run concurrently in background. Main and children must not duplicate an active track's evidence gathering, edits, or checks, and Workers must not repeat equivalent scans after evidence suffices. A result missing status or coverage remains incomplete. An eligible built-in delegation that omits `run_in_background` must use the profile's background default, and `max_turns` must remain omitted unless the user or current calibrated policy supplies it. Any ordinary foreground spawn, premature parent wait, idle parent, overlapping deliverable, capability/trust-boundary violation, accepted incomplete result, or silent unknown-type fallback blocks release. Repeat the explicit user-requested foreground and turn-limit cases to confirm both narrow overrides remain available.
16. In fresh CLI and Pi Web sessions, configure an unavailable exact title model followed by a fuzzy small model, submit prose around a large fenced log, and confirm the main model starts concurrently, the second model produces an 80-character-or-shorter persisted title, and a simultaneous manual name is never overwritten. Repeat with an empty model list to confirm active-model fallback.
17. Complete `/goal` once with a multiline direct user-facing deliverable and once with a rejected stale-id call in Pi Web and standalone Pi. Confirm the accepted summary appears after the collapsed Process details as an expanded durable `Goal complete` message, survives reload, does not trigger another provider request, uses Pi Web's 1.5-line-height compaction Markdown typography while another expanded generic custom message retains its upstream style, and includes the deliverable rather than promising a later response; confirm the rejected result remains only in Process details.
...
### Revision-10 prompt compression

Revision 10 removes repeated parent-facing routing text while preserving the revision-9 policy. The canonical parent policy is capped at 180 words, the three profile descriptions at 75 combined words, and their task recipes at 60 combined words. It still tells main to keep working when only one agent runs and explicitly sets `run_in_background: true` for default delegation; naming a profile does not authorize foreground execution, and the routing fixture separately scores named-profile and omitted-argument cases as background. New information remains a valid reason to retry or resume, and custom-profile descriptions remain eligible routing evidence after the gate. Each built-in child prompt now states its exact tool allowlist. Its inherited prefix preserves authority and project instructions while removing parent-only capability/routing metadata and the inherited skill catalog; the child loader appends skills once. Published-artifact verification renders that realistic shape, checks capability consistency and fixed prompt size, and confirms one effective skill catalog. Model, reasoning, execution-mode, and resume behavior remains beside the corresponding schema parameters. Revision-9 migration and uninstall coverage verifies the preceding owned shape.

### Revision-11 capability and resume preflight

Revision 11 adds a capability preflight before both spawn and resume. The selected profile's listed tools and supplied readable inputs must suffice; named commits, PRs, and Git refs require supplied diff text or a readable target checkout for built-in Explore or Research, while local Git, builds, tests, and generated-output experiments use Worker or stay in main after the parallelism gate. A follow-up that becomes the sole critical path stays in main, and resume documentation reflects the runtime fact that spawn-only parameters are ignored. Narrow or bounded questions receive finite `max_turns`. Explore checks revision accessibility before searching, stops rather than mining unrelated Git internals or caches, and reports evidence rather than verdicts. Worker and Research report unavailable execution prerequisites instead of proposing unexecuted experiments. The parent policy remains below 240 words, each authored child prompt remains below 275 words, and published-artifact verification retains the 400-word effective fixed-prompt cap. The complete parent-facing interface cap rises deliberately from 650 to 680 words to retain the capability, resume, and bounded-turn controls beside the tool schema. Revision-10 migration and uninstall coverage verifies the preceding owned shape. The routing fixture records the three observed regression shapes; matched fresh-session trials using `tests/fixtures/pi-subagents-routing-eval.json` remain a manual release requirement.

### Revision-12 deliverable ownership and stopping

Revision 12 gives every delegated track one distinct deliverable with explicit scope, exclusions, evidence or output, and a stopping condition. A lone background child is eligible only while main immediately performs substantial non-overlapping work; neither main nor another child repeats an active track's evidence gathering, edits, or checks. Architecture, planning, severity, acceptance, and verdicts remain with main. Built-in children stop when their scoped result is sufficient and report `complete`, `incomplete`, or `blocked` track status with unfinished coverage. Parent guidance treats stopped, limit-steered, missing-coverage, and residual-work results as incomplete. `max_turns` is now a circuit breaker set only by an explicit user limit or a calibrated value supplied in current instructions, not a complexity estimate. The schema-3 routing fixture adds overlapping-parent, delegated-verdict, sole-critical-path audit, omitted-limit, and explicit-limit cases. Static assertions and published-artifact rendering verify prompt identity and size; matched fresh-session trajectories remain required before attributing behavioral improvement.

### Revision-13 result integrity and decision ownership

Revision 13 addresses the observed lost-result incident: two Explore children previously consumed quota, returned `status: completed` with empty results, and left the parent with no payload and an apparent stall after its own provider error failed silently. The turn loop now inspects the final assistant message after `session.prompt()` resolves; a final `stopReason: "error"` fails the child with the provider error text plus any partial streamed or message output, and an otherwise successful blank response throws instead of becoming a completed empty result. `renderReportBody` falls back to "No output." for an empty trimmed string. A soft turn-limit wrap-up (`steered`) now counts as terminal-incomplete state and renders as "Incomplete (turn limit)" instead of "completed (steered)"; explicit invocation `max_turns` overrides an agent-profile default. The parent and child prompts were further compressed without losing controls: children never choose, recommend, rank, approve, or decide and evidence-only prompts mark decisions out of scope, while the parent rules restate disjoint ownership, active non-duplication, wait-or-reserved-work behavior, and incomplete-result determination in 220 words under the 240 cap. The schema-4 routing fixture adds parent-waiting, capability-first Git routing, reframed decision questions, and empty-child-result cases. Published-artifact verification executes the patched session lifecycle against provider error, partial error, empty, and successful responses and retains the 400-word effective fixed-prompt and 680-word parent-interface caps; matched fresh-session trajectories under step 15 remain required before attributing behavioral improvement.

### Revision-14 concrete ownership and handoff acceptance

Revision 14 addresses the ownership overlap and weak realized concurrency observed in session `01a06e0f-da10-7e50-8421-c5e64cbe9e9e`. A child dispatch now names owned and excluded files, sources, or actions; a lone child also requires named concurrent main work. Main performs that reserved work before waiting and does not duplicate active child evidence, edits, or checks. Worker stops repeating equivalent scans when its evidence is sufficient. Every built-in handoff field is required, with `none` used when empty, and the parent treats missing status or coverage as incomplete. The parent contract remains 220 words, the prompt-parameter description does not grow, and the authored Worker, Explore, and Research prompts remain within 275 words. The schema-5 routing fixture adds a Git-backed release-inventory trajectory with disjoint version-file ownership and a wait only after main completes its reserved work. Revision-13 migration and uninstall coverage preserves exact ownership. Matched fresh-session evaluation under step 15 remains required before attributing behavioral improvement.
