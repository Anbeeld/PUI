...
| Subagents | `@gotgenes/pi-subagents` | Pi package entry + PUI-owned source patch + user mapping config | `Worker` execution, local read-only `Explore`, external read-only `Research`, fail-closed routing, 128 running/512 queued background agents per Pi instance, completion delivery, model mapping, and reasoning inheritance |
