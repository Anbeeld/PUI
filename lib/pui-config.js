...
function validMappingRecord(value) {
  return isPlainObject(value) && Object.entries(value).every(([parent, child]) =>
    parent.trim() !== "" && typeof child === "string" && child.trim() !== "");
}
...
function reconcileModelMappings(existing, defaults) {
  ...
  for (const [parent, previousChild] of Object.entries(previousDefaults)) {
    if (Object.hasOwn(defaults, parent)) continue;
    if (current[parent] === previousChild) delete current[parent];
  }
  for (const [parent, child] of Object.entries(defaults)) {
    if (!Object.hasOwn(previousDefaults, parent)) {
      if (!Object.hasOwn(current, parent)) current[parent] = child;
    } else if (Object.hasOwn(current, parent) && current[parent] === previousDefaults[parent]) {
