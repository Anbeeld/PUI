...
//   ensure-reasoning-summary-modes <file> <json-object>  Create default modes only when missing.
//   validate-reasoning-summary-modes <file>       Validate the PUI reasoning-summary schema.
//   ensure-session-titles <file> <json-array>      Create the title-model list only when missing.
//   validate-session-titles <file>                 Validate the PUI session-title schema.
//
// Merge semantics:
...
function validateReasoningSummaryModesConfig(value) {
  if (!isPlainObject(value)) return { ok: false, error: "Top-level JSON must be an object" };
  if (value.schemaVersion !== 1) return { ok: false, error: "schemaVersion must be 1" };
  if (!isPlainObject(value.modelModes)) return { ok: false, error: "modelModes must be an object" };
  for (const [model, mode] of Object.entries(value.modelModes)) {
    if (model === "" || model !== model.trim() || !REASONING_SUMMARY_MODES.has(mode)) {
      return { ok: false, error: "modelModes must contain non-empty model keys and auto, concise, detailed, or none values" };
    }
  }
  return { ok: true };
}

function validateSessionTitlesConfig(value) {
  if (!isPlainObject(value)) return { ok: false, error: "Top-level JSON must be an object" };
  if (value.schemaVersion !== 1) return { ok: false, error: "schemaVersion must be 1" };
  if (!Array.isArray(value.models)) return { ok: false, error: "models must be an array" };
  const seen = new Set();
  for (const selector of value.models) {
    if (typeof selector !== "string" || selector === "" || selector !== selector.trim() || seen.has(selector.toLowerCase())) {
      return { ok: false, error: "models must contain non-empty, unpadded, case-insensitively unique strings" };
    }
    seen.add(selector.toLowerCase());
  }
  return { ok: true };
}
...
function main() {
  ...
  if (cmd === "validate-reasoning-summary-modes") {
    const file = expand(args[1]);
    const r = readJsonSafe(file);
    if (!r.ok || !r.existed) { console.error(`INVALID: ${file}: ${r.error || "file is missing"}`); process.exit(1); }
    const validated = validateReasoningSummaryModesConfig(r.value);
    if (!validated.ok) { console.error(`INVALID: ${file}: ${validated.error}`); process.exit(1); }
    console.log("VALID");
    return;
  }

  if (cmd === "validate-session-titles") {
    const file = expand(args[1]);
    const r = readJsonSafe(file);
    if (!r.ok || !r.existed) { console.error(`INVALID: ${file}: ${r.error || "file is missing"}`); process.exit(1); }
    const validated = validateSessionTitlesConfig(r.value);
    if (!validated.ok) { console.error(`INVALID: ${file}: ${validated.error}`); process.exit(1); }
    console.log("VALID");
    return;
  }

  if (cmd === "ensure-session-titles") {
    const file = expand(args[1]);
    const models = JSON.parse(readArg(args[2]));
    const candidate = { schemaVersion: 1, models };
    const defaultsValidation = validateSessionTitlesConfig(candidate);
    if (!defaultsValidation.ok) { console.error(`ERROR: ${file}: invalid defaults: ${defaultsValidation.error}`); process.exit(1); }
    const r = readJsonSafe(file);
    if (!r.ok) { console.error(`ERROR: ${file}: ${r.error}`); process.exit(1); }
    if (r.existed) {
      const validated = validateSessionTitlesConfig(r.value);
      if (!validated.ok) { console.error(`ERROR: ${file}: ${validated.error}`); process.exit(1); }
      process.stdout.write(JSON.stringify({ ok: true, file, changed: false }) + "\n");
      return;
    }
    writeJson(file, candidate);
    process.stdout.write(JSON.stringify({ ok: true, file, changed: true }) + "\n");
    return;
  }

  if (cmd === "ensure-reasoning-summary-modes") {
    const file = expand(args[1]);
    const defaults = JSON.parse(readArg(args[2]));
    const candidate = { schemaVersion: 1, modelModes: defaults };
    const defaultsValidation = validateReasoningSummaryModesConfig(candidate);
    if (!defaultsValidation.ok) { console.error(`ERROR: ${file}: invalid defaults: ${defaultsValidation.error}`); process.exit(1); }
    const r = readJsonSafe(file);
    if (!r.ok) { console.error(`ERROR: ${file}: ${r.error}`); process.exit(1); }
    if (r.existed) {
