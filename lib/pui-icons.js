...
const PRECACHED_ICON_NAMES = ["icon-192.png", "icon-512.png", "apple-touch-icon.png"];
const APPLE_ICONS = [
  ["icon-120.png", 120],
  ["icon-152.png", 152],
  ["icon-167.png", 167],
...
function versionedUrl(url) {
  const version = /([?&])v=[^&]+/;
  if (version.test(url)) return url.replace(version, `$1v=${ICON_VERSION}`);
  return `${url}${url.includes("?") ? "&" : "?"}v=${ICON_VERSION}`;
}

function iconUrl(name) {
  return versionedUrl(`/icons/${name}`);
}

function iconDescriptor(name, size) {
  return `{url:"${iconUrl(name)}",sizes:"${size}x${size}",type:"image/png"}`;
}
...
function htmlLinks(rel, icons) {
  return icons
    .map(
      ([name, size]) =>
        `<link rel="${rel}" href="${iconUrl(name)}" sizes="${size}x${size}" type="image/png"/>`,
    )
    .join("");
}
...
function rscLinks(rel, icons, escaped) {
  ...
  return icons
    .map(([name, size]) =>
      encode(
        `["$","link","pui-${rel}-${size}",{"rel":"${rel}","href":"${iconUrl(name)}","sizes":"${size}x${size}","type":"image/png"}]`,
      ),
    )
    .join(",");
}
...
function patchRscLinks(content) {
  ...
  let result = content.replace(
    ...
    (match, key, href) => {
      ...
      const encoded = `["$","link","${key}",{"rel":"icon","href":"${versionedUrl(href)}","type":"image/png","sizes":"32x32"}]`;
  ...
  result = replaceGroup(
    result,
    "icon",
    String.raw`\/icons\/icon-\d+\.png(?:\?v=[^"]+)?`,
    BROWSER_SIZES.map((size) => [`icon-${size}.png`, size]),
  );
  result = replaceGroup(
    result,
    "apple-touch-icon",
    String.raw`\/icons\/(?:icon-\d+|apple-touch-icon)\.png(?:\?v=[^"]+)?`,
    APPLE_ICONS,
  );
  return result;
}
...
function patchHtml(content) {
  let result = content.replace(
    ...
    (match, href) => `<link rel="icon" href="${versionedUrl(href)}" type="image/png" sizes="32x32"/>`,
  );
  result = result.replace(
    /(?:<link rel="icon" href="\/icons\/icon-\d+\.png(?:\?v=[^"]+)?" sizes="\d+x\d+" type="image\/png"\/>)+/g,
    htmlLinks(
      "icon",
      BROWSER_SIZES.map((size) => [`icon-${size}.png`, size]),
    ),
  ...
  result = result.replace(
    /(?:<link rel="apple-touch-icon" href="\/icons\/(?:icon-\d+|apple-touch-icon)\.png(?:\?v=[^"]+)?" sizes="\d+x\d+" type="image\/png"\/>)+/g,
...
function manifestIcons() {
  return [
    ...PWA_SIZES.map((size) => ({
      src: iconUrl(`icon-${size}.png`),
      sizes: `${size}x${size}`,
      type: "image/png",
      purpose: "any",
    })),
    ...MASKABLE_SIZES.map((size) => ({
      src: iconUrl(`maskable-${size}.png`),
      sizes: `${size}x${size}`,
      type: "image/png",
      purpose: "maskable",
    })),
...
function patchServiceWorkerIcons(content) {
  let result = content;
  for (const name of PRECACHED_ICON_NAMES) {
    const escapedName = name.replaceAll(".", "\\.");
    const pattern = new RegExp(`(["'])/icons/${escapedName}(?:\\?v=[^"']*)?\\1`, "g");
    result = result.replace(pattern, (_match, quote) => `${quote}${iconUrl(name)}${quote}`);
  }
  return result;
}
...
function applyIcons(assetDirectory, piWebRoot) {
  ...
  changed += Number(patchTextFile(path.join(root, "public", "sw.js"), patchServiceWorkerIcons));
