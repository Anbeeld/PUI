#!/usr/bin/env node
// PUI-managed, version-anchored model-guidance patch for
// @99percentpeople/pi-background-tasks.
//
// The upstream 2.1.1 bundle registers six tools with 27 overlapping system-
// prompt guidelines plus verbose descriptions and schema text. PUI replaces
// that metadata at its registration source, preserving tool execution and
// runtime validation. Install/update reapply the patch after `pi install`;
// uninstall restores the original only while the installed output still
// exactly matches the PUI-owned transform.
...
function hasCompactMetadata(text) {
  if (countOccurrences(text, SENTINEL) !== 1) return false;
  for (const metadata of Object.values(PUI_TOOL_METADATA)) {
    if (countOccurrences(text, JSON.stringify(metadata.description)) + countOccurrences(text, singleQuoted(metadata.description)) !== 1) return false;
    if (countOccurrences(text, JSON.stringify(metadata.promptSnippet)) + countOccurrences(text, singleQuoted(metadata.promptSnippet)) !== 1) return false;
  }
  for (const guideline of PUI_GUIDELINES) {
    if (countOccurrences(text, JSON.stringify(guideline)) + countOccurrences(text, singleQuoted(guideline)) !== 1) return false;
  }
  for (const targets of Object.values(GUIDELINE_TARGETS)) {
    if (countOccurrences(text, `promptGuidelines:${JSON.stringify(targets)}`) !== 1) return false;
  }
