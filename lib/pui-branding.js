...
// Pi Web renders every tool call header as `toolName summary`, where the
// summary only recognizes bash/read-style argument names ("command", "path",
// ...) and otherwise stringifies the first argument. For load_skill the first
// argument is the skill name, so a skill load and a reference load display
// identically. Inserting a branch for tools that pass both `name` and
// `reference` makes the header show `skill: ref1, ref2`. The seam exists in
// both the client page chunk and the server page bundle with different
// minified identifiers, so the pattern is identifier-agnostic and anchored at
// the summary chain's start to stay idempotent. Display-only patch.
const TOOL_CALL_SUMMARY_PATTERN =
  /(0===[A-Za-z_$][\w$]*\.length\?"":)"command"in ([A-Za-z_$][\w$]*)\?String\(\2\.command\)\.slice\(0,120\)/g;

function replaceToolCallSummary(content) {
  return content.replace(
    TOOL_CALL_SUMMARY_PATTERN,
    '$1"reference"in $2&&"name"in $2?[String($2.name),[].concat($2.reference).join(", ")].join(": ").slice(0,120):"command"in $2?String($2.command).slice(0,120)',
  );
}
...
function applyBranding(piWebRoot) {
  ...
  const replaceDisplay = (content) => replaceToolCallSummary(replaceBranding(content));
  let changed = 0;
  for (const file of cssFiles) changed += Number(patchFile(file, "utf8", replaceCssLayout));
  for (const file of new Set(textFiles)) changed += Number(patchFile(file, "utf8", replaceDisplay));
  for (const file of rscFiles) changed += Number(patchFile(file, "latin1", replaceDisplay));
  for (const file of clientFiles) changed += Number(patchFile(file, "utf8", replaceDisplay));
...
module.exports = { applyBranding, replaceBranding, replaceCssLayout, replaceToolCallSummary };
