...
const ownedFiles = ["index.ts", "updater.js", "pui-release.js", "pui-config.js"];
...
function installed() {
  ...
  if (manifest.owner !== "PUI" || manifest.schemaVersion !== 1 || !/^\d+\.\d+\.\d+$/.test(manifest.puiVersion || "")
    || JSON.stringify(Object.keys(manifest).sort()) !== JSON.stringify(expectedKeys) || !manifest.files || Array.isArray(manifest.files)
    || JSON.stringify(Object.keys(manifest.files).sort()) !== JSON.stringify([...ownedFiles].sort())
    || manifest.identityHash !== expected) throw new Error("Installed PUI identity is invalid");
  const root = path.resolve(extensionRoot);
  if (JSON.stringify(fs.readdirSync(root).sort()) !== JSON.stringify([...ownedFiles, "manifest.json"].sort())) throw new Error("Installed PUI identity file shape is invalid");
  for (const [relative, expectedHash] of Object.entries(manifest.files)) {
    if (typeof relative !== "string" || !relative || path.isAbsolute(relative) || relative.includes("\\") || relative.split("/").includes("..") || path.posix.normalize(relative) !== relative || !/^[a-f0-9]{64}$/.test(expectedHash)) throw new Error("Installed PUI identity contains an invalid file entry");
    const target = path.resolve(root, relative);
    if (!target.startsWith(`${root}${path.sep}`) || !fs.existsSync(target) || !fs.statSync(target).isFile()) throw new Error(`Installed PUI identity file is missing: ${relative}`);
    if (crypto.createHash("sha256").update(fs.readFileSync(target)).digest("hex") !== expectedHash) throw new Error(`Installed PUI identity file hash mismatch: ${relative}`);
  }
  return manifest;
}
...
async function getUpdate() {
  try {
    ...
    if (fs.existsSync(STATUS_FILE)) {
      ...
      // Restart statuses have no target. Active and failed states remain actionable;
      // a completed restart is delivered once, then removed so it cannot suppress
      // normal GitHub update discovery after the client reloads.
      if (!status.target && (status.phase === "restarting" || status.result === "restarted" || (status.phase === "failed" && status.result === "failed"))) {
        if (status.result === "restarted" && fs.readFileSync(STATUS_FILE, "utf8") === statusText) fs.unlinkSync(STATUS_FILE);
...
function acknowledge() {
  ...
  if (["success", "rolled-back", "aborted", "restarted"].includes(status.result)
    || (!status.target && status.phase === "failed" && status.result === "failed")) fs.unlinkSync(STATUS_FILE);
