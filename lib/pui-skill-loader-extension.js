...
const OWNED_FILES = ["core.ts", "index.ts", "package.json", "pui-extension-transaction.cjs"];
...
function defaultTarget() {
  return path.join(os.homedir(), ".pi", "agent", "extensions", "pui-skill-loader");
}

const directoryTransaction = createDirectoryTransaction({
  expectedFiles: [...OWNED_FILES, "manifest.json"],
  defaultTarget,
  guardName: "pui-skill-loader-guard.json",
});

function configuredTarget(repoRoot) {
  const stack = JSON.parse(fs.readFileSync(path.join(repoRoot, "stack.json"), "utf8"));
  const configured = stack.skillLoaderExtension?.target;
  if (typeof configured !== "string") throw new Error("stack.json skillLoaderExtension.target is missing");
  return path.resolve(configured.replace(/^~(?=$|[\\/])/, os.homedir()));
}

function sourceFiles(repoRoot) {
  return Object.fromEntries(OWNED_FILES.map((name) => [name, path.join(repoRoot, "extensions", "pui-skill-loader", name)]));
}
...
function installExtension({ repoRoot, target = defaultTarget() }) {
  if (path.basename(path.resolve(target)).toLowerCase() !== "pui-skill-loader") throw new Error(`Refusing unexpected extension target: ${target}`);
  ...
  const staged = fs.mkdtempSync(path.join(parent, ".pui-skill-loader-stage-"));
...
function main(argv) {
  ...
  try {
    ...
    if (["install", "verify", "remove"].includes(command)) {
      ...
      console.error("Usage: pui-skill-loader-extension.js <install|verify|remove> [repo-root] [target] | <snapshot|restore-snapshot> <state-dir> [repo-root] | <spawn-guard> <state-dir> <target-version> [repo-root]");
      return 64;
    }
