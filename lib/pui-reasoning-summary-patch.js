...
function patchPiWebLiveCustomMessages(text) {
  ...
  const connectedPattern = /case"connected":([A-Za-z_$][\w$]*)\(\{type:"end"\}\),!0===([A-Za-z_$][\w$]*)\.isStreaming&&\((.*?)\);break;case"agent_start":/g;
  ...
  const promptDonePattern = new RegExp(`case"prompt_done":\\{[^}]*?let ([A-Za-z_$][\\w$]*)=${session}\\.current;\\1(?:&&!${active}\\.current)?&&${loadSession}\\(\\1\\),!${active}\\.current&&`, "g");
  ...
  const promptDonePatched = promptDone[0].includes(`${promptDone[1]}&&!${active}.current&&${loadSession}(${promptDone[1]})`);
  if (connectedPatched || messageStartPatched || userDisarmPatched || promptDonePatched) {
    if (connectedPatched && messageStartPatched && userDisarmPatched && promptDonePatched) return { ok: true, changed: false, reason: "already-patched", text };
    fail("target-drift", "Pi Web live custom-message reconciliation has partial-patch drift");
  }

  const connectedReplacement = connected[0].replace(connected[3], `${connected[3]},!${rpcPromptPending}.current&&(${pending}=!0)`);
  const promptDoneReplacement = promptDone[0].replace(`${promptDone[1]}&&${loadSession}(${promptDone[1]})`, `${promptDone[1]}&&!${active}.current&&${loadSession}(${promptDone[1]})`);
