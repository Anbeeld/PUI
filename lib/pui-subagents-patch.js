...
const PROMPT_SNIPPET = "Launch a specialized agent for bounded work.";
const DESCRIPTION_SENTENCE = "Launch a new agent to handle complex, multi-step tasks autonomously.";
...
const EXPLORE_PROMPT = `You are a read-only codebase investigator working for a parent agent.

Answer the delegated question from evidence in the existing local repository. ${DECISION_OWNERSHIP_SENTENCE}
...
const RESEARCH_PROMPT = `You are a read-only external research subagent working for a parent agent.
...
const PROMPT_PARAMETER_DESCRIPTION = "The delegated task. Follow the selected agent type's prompt recipe in Guidelines.";
...
const PARALLEL_BOUNDARY_GUIDELINE = '"- Run background agents in parallel only when their tracks are independent and do not overlap writes or other shared mutable state.",';
...
const BACKGROUND_PARAMETER_DESCRIPTION = "Execution mode for a new agent: true returns an agent ID immediately and runs in background; false waits for the result. Omit to use the profile default (PUI built-ins: false).";
...
const EXECUTION_MODE_GUIDELINE = '"- Foreground calls wait for their result and run sequentially. Use foreground when the next parent action depends on that result; use run_in_background: true when useful independent work can proceed before collection.",';
const PATCHED_PARENT_GUIDELINE_ANCHOR = [
  EXECUTION_MODE_GUIDELINE,
  PARALLEL_BOUNDARY_GUIDELINE,
