...
const DESCRIPTION_SENTENCE = "By default, launch a background specialist only for a substantial independent track that can run alongside main work or another agent.";
...
const WORKER_DESCRIPTION = "After the parallelism gate, executes a decided independent background track and may edit files or run commands. Not for unresolved decisions or the overall plan.";
const EXPLORE_DESCRIPTION = "After the parallelism gate, gathers read-only local repository evidence for an independent background track. Not for implementation or the overall plan.";
const RESEARCH_DESCRIPTION = "After the parallelism gate, gathers read-only external or current evidence for an independent background track. Not for implementation or the overall plan.";
const WORKER_GUIDELINE = "- Worker prompt: give owned files, decided actions, constraints, success criteria, validation, and output; writers need disjoint ownership.";
const EXPLORE_GUIDELINE = "- Explore prompt: give the evidence question, readable target inputs, breadth, and output; request facts and uncertainty, not verdicts.";
const RESEARCH_GUIDELINE = "- Research prompt: give claims, version/date, primary-source and citation needs, and freshness; request evidence and uncertainty, not verdicts.";
const WORKER_PROMPT = `You are a bounded execution worker for an independent delegated track.

Your available tools are exactly: read, bash, edit, write, grep, find, and ls.
...
Validate the result within scope using the most relevant available checks. If an assigned empirical check cannot run because a prerequisite is unavailable, report the exact missing prerequisite and evidence; do not replace execution with a proposed experiment. Avoid unrelated cleanup.
...
Your available tools are exactly: read, grep, find, and ls.
...
Check evidence accessibility before searching. Do not assume the current working tree represents a named commit, PR, or Git ref. Treat another revision as available only when the prompt supplies its diff or a readable checkout path. You cannot execute shell or Git commands, builds, or tests. Do not reconstruct inaccessible revisions from Git internals, caches, generated trees, or unrelated scratch directories. If required evidence is unavailable, stop early and name the exact input or capability needed. Never cite one revision as evidence for another.

Report facts, implications, and uncertainty—not severity, acceptance decisions, or the final verdict.
...
Your available tools are exactly: read, grep, find, ls, web_search, source_check, fetch_content, and get_search_content.
...
If adequate sources or version/freshness evidence are unavailable or conflicting, report the gap. Local command, build, or test requirements are gaps, not invitations to propose unexecuted experiments.
...
const MAX_TURNS_PARAMETER_DESCRIPTION = "Maximum number of agentic turns before stopping. Set a finite limit for a narrow or bounded question; omit only for an intentionally open-ended track. A max-turn stop is incomplete.";
